...
function MyApplications() {
  const { applications, updateApplications, handleDecrement } =
    useContext(DataContext);
  const { user } = useContext(AuthContext);

  console.log(applications); // Log application IDs

  const handleDelete = (_id) => {
    console.log('Deleting application with ID:', _id); // Log the ID being passed
    Swal.fire({
      title: 'Are you sure?',
      ...
      if (result.isConfirmed) {
        fetch(`http://localhost:4000/visaApplications/${_id}`, {
          method: 'DELETE',
        })
          .then((res) => res.json())
          .then((data) => {
